...
> **(b)** The empirical trade-off between  predictive risk and  unfairness， both mapped onto [0,1]. The kneedle algorithm rigorously identifies the point that maximizes the vertical distance to the reference chord.  
 ...
 **Table 5:** The mean and standard deviation of runtime (in seconds) over 30 repeats with varying sample sizes $n$ and $|\mathcal{S}|$ and $J_n=m_n+d$, where the variable $S$ is drawn uniformly at random from the discrete set $\{0, 1, \ldots, |\mathcal{S}|\}$. To show the computational cost of the quantile optimization, we obtain  estimator $\widehat{f}$ using the ordinary linear regression.  We set $d=3$ and the optimal rate to balance approximation bias and variance is achieved by setting $m_n=\left[n^{1 /(2 d+1)}\right]$  based on the standard asymptotic theory.
